...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    } else if (req.baseUrl.includes('/api/stories')) {
      uploadDir = path.join(__dirname, '../uploads/stories');
    } else {
...
const fileFilter = (req, file, cb) => {
  if (req.baseUrl.includes('/api/posts') || req.baseUrl.includes('/api/stories')) {
...
const upload = multer({
  storage,
  limits: { 
    fileSize: 50 * 1024 * 1024, // 50MB limit for posts and stories
    files: 1
  },
  fileFilter
});
